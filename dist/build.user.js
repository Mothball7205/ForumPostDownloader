        ...
        const tryVs = async base => {
          ...
          try {
            ...
            try {
              ...
              const hint = vsHint || bunkrNameByUrl.get(cleanUrl) || bunkrNameByUrl.get(strip(cleanUrl)) || '';
      ...
      const getWebsiteToken = async (force = false) => {
        ...
        for (const u of candidates) {
          try {
            ...
            const m = txt.match(/\bwt\s*=\s*"([^"]+)"/) || txt.match(/fetchData\.wt\s*=\s*"([^"]+)"/) || txt.match(/"wt"\s*:\s*"([^"]+)"/);
  ...
  if (!postSettings.skipDownload) {
    ...
    while (batch.length) {
      ...
      const applyFilesterAlbumPolicy = async () => {
        try {
          const isFilesterAlbumOriginal = s =>
            /(?:^|\/\/)(?:www\.)?filester\.me\/f\//i.test(String(s || '')) || /filester\.me\/f\//i.test(String(s || ''));
      ...
      const startDownload = async (resource, pass = 1) => {
        ...
        const isFilester =
          String((host && host.name) || '').toLowerCase() === 'filester' || /(?:^|\.)filester\.me/i.test(String(url || ''));
        ...
        if (
          ...
          cyberOrigin &&
          cyberFilePage &&
          /gigachad-cdn\.ru|selti-delivery\.ru/i.test(String(url || '')) &&
                  ...
                  if (badStatus) {
                    ...
                    if (isRetryable) {
                      ...
                      if (token0) {
                        ...
                        if (isOn6) {
                          for (const c of candidates0 || []) {
                            if (/https?:\/\/cache1\.filester\.me\//i.test(c) && !tried0.has(c)) {
                              nextUrl = c;
                              tried0.add(c);
                              break;
                            }
                          ...
                          for (const c of candidates0 || []) {
                            if (/https?:\/\/cache6\.filester\.me\//i.test(c) && !tried0.has(c)) {
                              nextUrl = c;
                              tried0.add(c);
                              break;
                            }
...
const addDuplicateTabLink = post => {
  ...
  const template = post.parentNode.querySelector('.u-concealed');
  const attribution = post.parentNode.querySelector('.message-attribution-main');

  if (!template || !attribution || attribution.querySelector('.duplicate-tab')) {
    return null;
  }
  ...
  const dupTabLI = template.cloneNode(true);
  ...
  if (anchor.parentNode?.style) {
    anchor.parentNode.style.marginLeft = '10px';
  }
  anchor.append(span);

  attribution.append(dupTabLI);
  return dupTabLI;
};
...
const addShowDownloadPageBtnLink = post => {
  ...
  if (!template || !attribution || attribution.querySelector('.show-download-page')) {
    return null;
  }
...
const addDownloadPageButton = () => {
  const existingButton = h.element('#download-page');
  if (existingButton) {
...
(function () {
  ...
  try {
    if (window.__XFPD_BOOTSTRAPPED) return;
  } catch (e) {}
  ...
  const bootstrap = async () => {
    try {
      window.__XFPD_BOOTSTRAPPED = true;
    } catch (e) {}

    const postsReady = await h.waitForElement('.message-attribution-opposite');
    if (!postsReady) {
      console.warn('XFPD: thread posts were not found before startup timeout.');
      return;
    }
    ...
    h.elements('.message-attribution-opposite').forEach(post => {
      if (!post || post.querySelector('[data-xfpd-post-download="1"]')) {
        return;
      }
      ...
      if (!contentContainer || typeof contentContainer.prepend !== 'function') {
        return;
      }
    ...
    if (parsedPosts.filter(p => p.parsedHosts.length).length > 0) {
      ...
      if (!btnDownloadPage) {
        console.warn('XFPD: page download button container was not found.');
        return;
      }
        ...
        onShown: () => {
          parsedPosts
            ...
            .forEach(post => {
              ...
              if (toggleAllCB && !toggleAllCB.dataset.xfpdBound) {
